...
class PC3DDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, FLAGS, split):
        ...
        points, label = torch.from_numpy(points).to(torch.float), torch.from_numpy(label)
    ...
    def __getitem__(self, idx):
        ...
        if self.split == 'test':
            r = RandomRotation()
            x_0 = r(torch.as_tensor(self.data['points'][idx], dtype=torch.float32)) #.astype(DTYPE))
        else:
            x_0 = self.data['points'][idx]
            if self.aug:
                x_0 = provider.rotate_point_cloud(x_0)
                x_0 = provider.jitter_point_cloud(x_0)
                x_0 = provider.shuffle_points(x_0)
